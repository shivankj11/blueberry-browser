...
const renderTextWithCitations = (
    ...
    return parts.map((part, i) => {
        if (i % 2 === 1) {
            ...
            return <CitationMarker key={`cite-${i}`} index={idx} citation={citation} />
        }
...
const Markdown: React.FC<{ content: string; citations?: Citation[] }> = ({ content, citations }) => (
    <div className="prose prose-sm dark:prose-invert max-w-none
        ...
        <ReactMarkdown
            ...
            components={{
                ...
                p: ({ children }) => (
                    <p>{processReactChildren(children, citations)}</p>
                ),
                li: ({ children, ...props }) => (
...
const CitationRefList: React.FC<{ citations: Citation[] }> = ({ citations }) => {
